# Local modules
import elecloc.utils as utils
from elecloc.utils import log
import elecloc.contacts_isolation as contacts_isolation
import elecloc.segmentation_multimodel as segmentation
import elecloc.postprocessing as postproc
import elecloc.validation as validation
import elecloc.plot as plot
from elecloc.electrode_models import (LinearElectrodeModel, 
...
def main():
    ...
    labels, models = segmentation.segment_electrodes(
        contacts, N_ELECTRODES, 
        model_cls=LinearElectrodeModel)

    ### Postprocessing
    log("Post-processing results")
    contacts, labels, contacts_ids, models = postproc.postprocess(
        contacts, labels, ct_center_world, models, electrodes_info,
        model_cls=ParabolicElectrodeModel)
    intercontact_dist_world = postproc.__estimate_intercontact_distance(contacts)
    ...
    intercontact_dist_vox = postproc.__estimate_intercontact_distance(contacts)
