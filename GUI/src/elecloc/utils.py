...
class NibCTWrapper:
    def __init__(self, ct_path: str, ct_brainmask_path: Optional[str]):
        """Creates a instance of Nibabel CT wrapper.
        ...
        - ValueError if either path exists but doesn't have a .nii.gz extension.
        ...
        if not ct_brainmask_path in [None, ""]:
            ...
            if self.ct.shape != self.mask.shape:
                format_shape = lambda arr: str(arr.shape[1:-1]).replace(", ", " x ")
                raise ValueError("The specified volumes (input and mask) should "
                                 "have different shapes but don't.\n"
                                 f"Input: {format_shape(self.ct)}\n"
                                 f"Mask:  {format_shape(self.mask)}")
        else:
    ...
    def apply_threshold(self, min: Optional[float], 
        ...
        elif min > self.ct.max():
            raise ValueError("Specified 'min' threshold greater "
                             "than CT highest value.")

        if max is None: max = self.ct.max()
        elif max < self.ct.min():
            raise ValueError("Specified 'max' threshold smaller" 
                             "than CT lowest value.")
